...
rename_to_snplg_file() {
    ...
    local project_name="$2"
    local out_dir="${zip_file_path%/*}"
    # Use the PLUGIN version (PluginConfig.json versionName, the value bumped each
    # build) for the filename -- not package.json's PACKAGE_VERSION, which is a
    # stale 0.0.1 default. Read it from the generated config the package was built
    # from; fall back to PACKAGE_VERSION if that lookup fails.
    local gen_cfg="${out_dir%/outputs}/generated/PluginConfig.json"
    local ver="$PACKAGE_VERSION"
    if command -v jq >/dev/null 2>&1 && [[ -f "$gen_cfg" ]]; then
        local v; v="$(jq -r '.versionName // empty' "$gen_cfg")"
        [[ -n "$v" ]] && ver="$v"
    fi
    # The INSTALLABLE file must be named exactly "<pluginKey>.snplg" -- Supernote's
    # installer verifies the filename against the package and rejects a renamed one
    # ("Installation Failed. The plugin might have been modified."). So the file the
    # user installs stays "Ink2Task.snplg".
    local installable="$out_dir/$project_name.snplg"
    cp "$zip_file_path" "$installable"
    # Keep a version-stamped copy for records (NOT for install -- the device rejects
    # non-canonical names). Tucked in archive/ so it's never grabbed by mistake.
    local archive_dir="$out_dir/archive"
    mkdir -p "$archive_dir"
    local versioned="$archive_dir/$project_name-${ver}.snplg"
    cp "$zip_file_path" "$versioned"
    write_color_output "Plugin package created (install this): $installable" "Green"
    write_color_output "Version archive: $versioned" "Green"
    echo "$installable"
}
...
main() {
    ...
    if [[ "$should_build_native" -eq 0 ]]; then
        # The APK build has to run *before* we read the autolinking package
        # list: PackageList.java is generated by Gradle, so on a clean build
        # it does not exist yet. Reading it first silently yields an empty
        # reactPackages array, the host never instantiates the native
        # packages, and every native module comes back null at runtime.
        ...
        # Native sources were detected above, so an empty list here means the
        # detection went wrong rather than that there is nothing to register.
        # Fail loudly -- a plugin packaged this way installs fine and then
        # dies on launch, which is a miserable thing to debug from the device.
        if [[ -z "$all_pkgs" ]]; then
            write_color_output "Native modules were detected but no ReactPackages were resolved." "Red"
            write_color_output "Packaging now would produce a plugin that crashes on launch. Aborting." "Red"
            exit 1
        fi
