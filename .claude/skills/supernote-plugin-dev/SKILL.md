...
> Vendored into Ink2Task from [AgP42/supernote-dashboard](https://github.com/AgP42/supernote-dashboard)
> (MIT, © 2026 AgP42 — see `LICENSE-upstream.txt`), itself adapted from
> [Laumss/Inkling](https://github.com/Laumss/Inkling) (MIT, © 2026 Laumss).
...
1. **Gotcha #9 says call `saveCurrentNote()` BEFORE `insertElements` /
   ...
   fix. Our order is: capture reads → `replaceElements` → **then**
   `saveCurrentNote` + `reloadFile`. See the comment in `plugin/src/utils/capture.ts`.

2. **Gotcha #4 (recycle elements) is correct and we follow it — via
...
3. **Gotcha #33 / setup-and-build §5b overstate the `reactPackages` problem for
...
4. **Pattern 6 / floating-window.md works on this device — confirmed v1.0.6.**
